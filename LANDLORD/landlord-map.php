...
$sql = "
    ...
    FROM listingtbl 
    WHERE landlord_id = ? AND latitude IS NOT NULL AND longitude IS NOT NULL
";
...
<body>
    ...
    <div class="landlord-page container-fluid px-4">
        <div class="mb-3">
            ...
            <p class="text-muted small mb-0" style="font-weight:600">
                Listings, hospitals, evacuation centers, fault line, and flood hazard overlay for <strong>San Pedro, Laguna</strong>.
            </p>
        </div>
        ...
        <div class="map-wrapper" style="position:relative;">
            <div class="map-sidebar">
                ...
                <div class="sidebar-section">
                    <div class="sidebar-section-title">My Listings</div>
                    <div class="stat-grid">
                ...
                <div class="sidebar-section">
                    ...
                    <div style="margin-top:4px;">
                        <span style="font-size:10px;color:#bbb;font-weight:700;">Buffer zone: ≥ 5m from fault trace</span>
                    </div>
                    <!-- Earthquake preparedness button -->
    ...
    // ── West Valley Fault — Main Trace (RED - Certain) ──
    // Traced from PHIVOLCS AFT-2021-043425-02 Ground Rupture Hazard Map
    const faultPath = [
        // North entry — Susana Heights border with Muntinlupa
        {lat:14.3875, lng:121.0362},
        {lat:14.3858, lng:121.0368},
        {lat:14.3842, lng:121.0373},
        {lat:14.3825, lng:121.0378},
        // San Antonio area — slight SE curve
        {lat:14.3808, lng:121.0382},
        {lat:14.3792, lng:121.0385},
        {lat:14.3775, lng:121.0388},
        {lat:14.3758, lng:121.0390},
        // Sitio Maharlika — curves slightly east
        {lat:14.3742, lng:121.0391},
        {lat:14.3725, lng:121.0392},
        {lat:14.3708, lng:121.0391},
        // GSIS area — starts curving west
        {lat:14.3692, lng:121.0388},
        {lat:14.3675, lng:121.0383},
        {lat:14.3658, lng:121.0377},
        // Sampaguita Village / Calendola
        {lat:14.3642, lng:121.0370},
        {lat:14.3625, lng:121.0362},
        {lat:14.3608, lng:121.0354},
        // Magsaysay / United Bayanihan
        {lat:14.3592, lng:121.0346},
        {lat:14.3575, lng:121.0338},
        {lat:14.3558, lng:121.0330},
        // Stonecrest / Nicolasa Virata
        {lat:14.3542, lng:121.0322},
        {lat:14.3525, lng:121.0314},
        {lat:14.3508, lng:121.0306},
        // Francisco Reyes / Gregoria de Jesus
        {lat:14.3492, lng:121.0298},
        {lat:14.3475, lng:121.0290},
        {lat:14.3458, lng:121.0282},
        // Exit south toward Cavite border
        {lat:14.3442, lng:121.0274},
        {lat:14.3425, lng:121.0266},
        {lat:14.3408, lng:121.0258},
        {lat:14.3390, lng:121.0250},
    ];

    // ── Secondary Fault Trace (ORANGE - Approximate) ──
    // Follows the approximate eastern trace visible in PHIVOLCS app
    const faultPathSecondary = [
        // Starts near San Antonio / Maharlika Drive junction
        {lat:14.3822, lng:121.0435},
        {lat:14.3805, lng:121.0440},
        {lat:14.3788, lng:121.0443},
        // Through San Vicente area
        {lat:14.3770, lng:121.0445},
        {lat:14.3752, lng:121.0447},
        {lat:14.3735, lng:121.0448},
        // Chrysanthemum
        {lat:14.3718, lng:121.0450},
        {lat:14.3700, lng:121.0452},
        {lat:14.3682, lng:121.0455},
        // Rosario
        {lat:14.3665, lng:121.0458},
        {lat:14.3648, lng:121.0462},
        {lat:14.3630, lng:121.0466},
        // Road 7 heading south
        {lat:14.3612, lng:121.0470},
        {lat:14.3595, lng:121.0474},
        {lat:14.3578, lng:121.0478},
        {lat:14.3560, lng:121.0482},
        {lat:14.3542, lng:121.0486},
        {lat:14.3525, lng:121.0490},
    ];
    ...
    let map, iw;
    let listingMarkers = [], hospitalMarkers = [], evacMarkers = [];
    ...
    function initMap() {
        ...
        evacCenters.forEach(ec => {
            const marker = new google.maps.Marker({
                ...
                icon: { path: google.maps.SymbolPath.CIRCLE, scale: 9, fillColor: '#2563eb', fillOpacity: 1, strokeColor: '#fff', strokeWeight: 2 },
                zIndex: 10,
            });
            marker.addListener('click', () => {
                iw.setContent(`<div class="popup-info"><p class="pi-title">⛺ ${ec.name}</p><p class="pi-sub">Evacuation Center · San Pedro, Laguna</p></div>`);
                iw.open(map, marker);
            });
            ...
            marker.addListener('click', () => {
                iw.setContent(`
                    <div class="popup-card">
                        ...
                        <p class="popup-sub">San Pedro, Laguna</p>
        ...
        document.getElementById('tog-listings').addEventListener('change', function() { setMarkers(listingMarkers, this.checked); });
        document.getElementById('tog-hospitals').addEventListener('change', function() { setMarkers(hospitalMarkers, this.checked); });
        document.getElementById('tog-evac').addEventListener('change',     function() { setMarkers(evacMarkers, this.checked); });
        document.getElementById('tog-border').addEventListener('change',   function() { borderPolygon.setMap(this.checked ? map : null); });
        document.getElementById('tog-flood').addEventListener('change',    function() { floodLayer.setMap(this.checked ? map : null); });
   ...
   content.innerHTML = `
    ...
    <div class="hazard-section-title">Flood Susceptibility Levels <span style="font-size:10px;font-weight:500;color:#999;">(PAGASA / PHIVOLCS-DOST)</span></div>
    ...
    <ul class="hazard-list">
        <li>Know your Barangay's evacuation route and nearest Evacuation Center (per BDRRMP).</li>
        <li>Prepare a Go-Bag for at least 72 hours of self-sufficiency.</li>
        <li>Store important documents (IDs, land titles, insurance) in a waterproof pouch or sealed bag.</li>
        <li>Monitor PAGASA weather bulletins, NDRRMC advisories, and your Barangay's official announcements.</li>
        <li>Elevate appliances and valuables off the ground floor if flooding is expected.</li>
        <li>Identify and secure LPG tanks and electrical main switches in advance.</li>
    </ul>

    <div class="hazard-section-title">During a Flood</div>
    <ul class="hazard-list">
        <li>Follow the Mandatory Evacuation Order issued by your Barangay or LGU immediately — do not wait.</li>
        <li>Never walk through moving floodwater — even 15cm of fast-moving water can knock an adult down.</li>
        <li>Never drive through flooded roads — 30cm of water can stall a vehicle; 60cm can sweep it away.</li>
        <li>Turn off electricity at the main breaker and shut off LPG supply valves before evacuating.</li>
        <li>If trapped and unable to evacuate, move to the highest floor — do not enter the attic unless you have a way out.</li>
        <li>Call 911 (National Emergency Hotline) or 8-PAGASA (8-724272) if you need rescue assistance.</li>
    </ul>
    ...
    <ul class="hazard-list">
        ...
        <li>Do not use tap water until local authorities confirm it is safe — floodwater may have contaminated supply lines.</li>
        <li>Wear rubber boots and gloves when cleaning; floodwater may contain sewage, chemicals, and pathogens.</li>
        <li>Disinfect all surfaces, utensils, food containers, and water storage using chlorine solution.</li>
        <li>Watch for leptospirosis symptoms (fever, muscle pain, red eyes) — seek medical attention immediately if symptoms appear.</li>
        <li>Document damage with photos for DSWD Assistance (AICS) and insurance claims.</li>
        <li>Report damage to your Barangay Disaster Risk Reduction and Management Council (BDRRMC).</li>
    </ul>

    <div class="hazard-section-title">Emergency Go-Bag Checklist <span style="font-size:10px;font-weight:500;color:#999;">(NDRRMC Standard)</span></div>
    <ul class="hazard-list">
        <li>Water: At least 1 liter per person per day (minimum 3-day supply)</li>
        <li>Food: Ready-to-eat, non-perishable items for at least 3 days (e.g., canned goods, energy bars)</li>
        <li>Flashlight with extra batteries or hand-crank flashlight</li>
        <li>Battery-operated or hand-crank AM/FM radio for PAGASA updates</li>
        <li>First-Aid Kit with prescribed medications (minimum 7-day supply)</li>
        <li>Waterproof pouch with copies of IDs, birth certificates, land titles, and insurance documents</li>
        <li>Cellphone with chargers, power bank, and load/credits</li>
        <li>Cash in small denominations</li>
        <li>Change of clothes (at least 2 sets) and sturdy footwear</li>
        <li>Emergency blanket(s), poncho or raincoat</li>
        <li>Whistle, dust mask, and local hazard map</li>
        <li>Written list of family members' contacts and nearest evacuation center address</li>
    </ul>
    ...
    <ul class="hazard-list">
        ...
        <li><strong>PAGASA 24/7 Hotline:</strong> 8-PAGASA (8-724272)</li>
        <li><strong>San Pedro, Laguna MDRRMO:</strong> Contact your Barangay Hall for local hotline</li>
        <li><strong>Philippine Red Cross:</strong> 143</li>
            ...
            content.innerHTML = `
                ...
                <p style="font-size:12px;color:#999;font-weight:600;margin:0 0 10px;">San Pedro, Laguna · West Valley Fault (PHIVOLCS)</p>
                ...
                <ul class="hazard-list">
                    ...
                    <li>Check for injuries and administer first aid as needed.</li>
                    ...
                    <li>Document structural damage before re-entering buildings.</li>
                </ul>
                <div class="hazard-section-title">Emergency Go-Bag Checklist</div>
                <ul class="hazard-list">
                    <li>Water: 1 gallon per person per day (minimum 3 days)</li>
                    <li>Food: Ready-to-eat items (3-day supply)</li>
                    <li>Whistle to signal for help if trapped</li>
                    <li>Dust mask / N95, work gloves, sturdy closed-toe shoes</li>
                    <li>First-Aid Kit with manual</li>
                    <li>Flashlight, extra batteries, portable radio</li>
                    <li>Copies of important documents in waterproof bag</li>
            ...
            content.innerHTML = `
                ...
                <ul class="hazard-list">
                    ...
                    <li>Keep updated via PAGASA, NDRRMC, and local government announcements.</li>
                </ul>
                ...
                <ul class="hazard-list">
                    ...
                    <li>Report injuries and damaged structures to barangay officials.</li>
                    <li>Watch out for secondary hazards: landslides, flash floods, disease outbreaks.</li>
                    <li>Boil all drinking water until declared safe.</li>
                </ul>
                <div class="hazard-section-title">Emergency Contacts</div>
                <ul class="hazard-list">
                    <li>NDRRMC Hotline: <strong>911</strong></li>
                    <li>San Pedro CDRRMO: <strong>(02) 8403-2649/0998 594 1743</strong></li>
                    <li>PAGASA: <strong>(02) 8284-0800</strong></li>
                    <li>Red Cross: <strong>143</strong></li>
                </ul>`;
        }
