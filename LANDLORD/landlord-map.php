    ...
    <div class="landlord-page container-fluid px-4">
        <div class="mb-3">
            ...
            <p class="text-muted small mb-0" style="font-weight:600">
                Browse available listings, fault line, and flood hazard overlay for <strong>San Pedro, Laguna</strong>.
            </p>
        </div>
...
listings.forEach(item => {
    ...
    const svgPin = `<svg xmlns="http://www.w3.org/2000/svg" width="28" height="36" viewBox="0 0 28 36">
        <path d="M14 0C6.268 0 0 6.268 0 14c0 10.5 14 22 14 22S28 24.5 28 14C28 6.268 21.732 0 14 0z" fill="${color}" stroke="#fff" stroke-width="2"/>
...
document.getElementById('stat-high').textContent  = statHigh;
        ...
        // ── Hospitals ──
const hospitals = [
    { name: "Westlake Medical Center",                lat: 14.347052534208073, lng: 121.06527443731535 },
    { name: "San Pedro Doctors Hospital",             lat: 14.348905619648429, lng: 121.06492326278244 },
    { name: "Divine Mercy Hospital",                  lat: 14.358766603058566, lng: 121.05828046253384 },
    { name: "Evangelista Medical Specialty Hospital", lat: 14.342008316605542, lng: 121.0583174919931 },
    { name: "Family Care Hospital",                   lat: 14.342391900731622, lng: 121.05817017167028 },
    { name: "San Pedro District Hospital",            lat: 14.363049724380952, lng: 121.0423068037687 },
    { name: "Jose L. Amante Emergency Hospital",      lat: 14.365025146606275, lng: 121.05593114813232 },
    { name: "LPH-San Pedro District Hospital",        lat: 14.363848651601888, lng: 121.04212523927708 },
    { name: "Lifestream Diagnostic Clinic, Inc.",     lat: 14.35083276460385, lng: 121.06360762628714 },
    { name: "Associated Doctors Clinic Corporation", lat: 14.34895122974907, lng: 121.06568043077394 }
];

hospitals.forEach(hospital => {
    const svgHospital = `<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30">
        <circle cx="15" cy="15" r="14" fill="#dc2626" stroke="#fff" stroke-width="2"/>
        <text x="15" y="20" text-anchor="middle" font-size="16" fill="#fff" font-family="Arial">🏥</text>
    </svg>`;
    const iconUrl = 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(svgHospital);
    ...
    marker.addListener('click', () => {
        iw.setContent(`
            <div class="popup-info">
                <p class="pi-title">${hospital.name}</p>
                <span class="pi-badge" style="background:#fee2e2;color:#991b1b;">🏥 Hospital</span>
                <p class="pi-sub">Medical facility<br>San Pedro, Laguna</p>
            </div>`);
        iw.open(map, marker);
    });
...
const evacCenters = [
    { 
        ...
        brgy: "Barangay Rosario", 
        lat: 14.336450167163804, 
        lng: 121.0490797375738 
    },
    { 
        name: "Landayan Covered Court", 
        brgy: "Barangay Landayan", 
        lat: 14.352244594895518, 
        lng: 121.0677307625243 
    },
    { 
        name: "Landayan Evacuation Center", 
        brgy: "Barangay Landayan", 
        lat: 14.35307978280281, 
        lng: 121.06605277788373 
    },
    { 
        name: "San Roque Evacuation Center (San Roque Elementary School)", 
        brgy: "Barangay San Roque", 
        lat: 14.366662252775479, 
        lng: 121.06159990498776 
    },
    { 
        name: "Cuyab Integrated High School", 
        brgy: "Barangay Cuyab", 
        lat: 14.371069496298231, 
        lng: 121.05914740205392 
    },
    { 
        name: "San Pedro Central Elementary School", 
        brgy: "Barangay Poblacion", 
        lat: 14.363929268526762, 
        lng: 121.05798359128791 
    },
    { 
        name: "Nueva Covered Court", 
        brgy: "Barangay Nueva", 
        lat: 14.361496248825922, 
        lng: 121.05217057339294 
    },
    { 
        name: "Sampaguita Village Elementary School", 
        brgy: "Barangay Sampaguita", 
        lat: 14.344949392933092, 
        lng: 121.03532510935828 
    },
    { 
        name: "Bagong Silang Elementary School", 
        brgy: "Barangay Bagong Silang", 
        lat: 14.34869322525628, 
        lng: 121.05562395649966 
    }
];
const evacSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36">
    <polygon points="18,2 34,34 2,34" fill="#0ea5e9" stroke="#fff" stroke-width="2" stroke-linejoin="round"/>
    <text x="18" y="29" text-anchor="middle" font-size="14" fill="#fff" font-family="Arial" font-weight="bold">EC</text>
</svg>`;
const evacIconUrl = 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(evacSvg);
...
evacCenters.forEach(center => {
    ...
    marker.addListener('click', () => {
        iw.setContent(`
            <div class="popup-info">
                <p class="pi-title">${center.name}</p>
                <span class="pi-badge" style="background:#e0f2fe;color:#0369a1;">🏠 Evacuation Center</span>
                <p class="pi-sub">
                    ${center.brgy}<br>San Pedro, Laguna<br>
                    <span style="color:#dc2626;font-weight:700;">Active during declared emergencies</span><br>
                    <strong>CDRRMO Hotline:</strong> 0998-594-1743
                </p>
            </div>`);
        iw.open(map, marker);
    });
...
faultPolylineSecondary = new google.maps.Polyline({
    ...
    map: null,   // hidden by default (controlled by tog-fault)
    zIndex: 5,
});
...
// Also set main fault + flood to hidden on init
faultPolyline.setMap(null);
...
document.getElementById('tog-hospitals').addEventListener('change', function () {
    setMarkers(hospitalMarkers, this.checked);
});
