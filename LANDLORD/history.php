...
$verifyQuery = "SELECT verification_status, admin_rejection_reason 
                FROM landlordtbl 
                WHERE ID = ?";
...
<?php
exit; // stop loading the rest of the dashboard
?>
                                ...
                                <td> 
                                    <?php if (!empty($tenant['profilePic'])): ?> 
                                        <a href="tenant-profile.php?tenant_id=<?= $tenant['tenant_id'] ?>"> 
                                            <img src="../uploads/<?= htmlspecialchars($tenant['profilePic']) ?>" alt="<?= htmlspecialchars($tenant_name) ?>" 
                                            class="rounded-circle" width="40" height="40" style="object-fit: cover; border: 2px solid #8d0b41;"> </a> 
                                            <?php else: ?> <div class="profile-avatar-sm"><?= $tenant_initial ?></div>
                                    <?php endif; ?> 
                                </td>
                            ...
                            <?php foreach ($complaints as $complaint):
                                ...
                                <tr>
                                    ...
                                    <td><span class="text-muted"><i
                                                class="fas fa-home me-1"></i><?= htmlspecialchars($complaint['property_name']) ?></span>
                                    </td>
                                        ...
                                        <?php elseif ($status === 'rejected'): ?>
                                            ...
                                            <div class="d-flex gap-2">
                                                <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal"
                                                    data-bs-target="#complaintModal" data-id="<?= $complaint['complaint_id'] ?>"
