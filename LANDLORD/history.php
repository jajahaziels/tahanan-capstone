...
/* ─── COMPLAINTS — using exact DB column names ─── */
...
function getStatusBadge($status) {
    return match (strtolower($status)) {
        ...
        'in progress' => '<span class="badge badge-scheduled">In Progress</span>',
...
.header-live-badge { display:inline-flex; align-items:center; gap:7px; background:var(--maroon); color:#fff; padding:8px 18px; border-radius:40px; font-size:.8rem; font-weight:700; white-space:nowrap; box-shadow:0 4px 12px rgba(141,11,65,.25); }
...
.stat-card { background:var(--surface); border-radius:var(--radius); border:1px solid var(--border); border-left:5px solid transparent; padding:20px 20px 16px 18px; transition:transform .2s, box-shadow .2s; box-shadow:var(--shadow); cursor:default; }
...
.rm-table thead th { padding:12px 18px; font-size:.68rem; font-weight:800; color:var(--ink3); text-transform:uppercase; letter-spacing:.07em; background:var(--surface2); border-bottom:1px solid var(--border); white-space:nowrap; text-align:left; }
...
.avatar { width:38px; height:38px; border-radius:50%; background:linear-gradient(135deg, var(--maroon) 0%, var(--maroon-dk) 100%); color:#fff; display:flex; align-items:center; justify-content:center; font-weight:900; font-size:.82rem; flex-shrink:0; border:2px solid #fff; box-shadow:0 2px 8px rgba(141,11,65,.18); overflow:hidden; }
...
.tenant-link { display:inline-flex; align-items:center; gap:11px; text-decoration:none; color:inherit; border-radius:8px; padding:3px 8px 3px 3px; transition:background .15s; margin:-3px -8px -3px -3px; }
...
.btn { display:inline-flex; align-items:center; justify-content:center; gap:5px; height:32px; padding:0 13px; font-size:.78rem; font-weight:700; border-radius:var(--radius-sm); border:none; cursor:pointer; transition:all .18s; white-space:nowrap; text-decoration:none; font-family:'Montserrat', sans-serif; line-height:1; letter-spacing:.01em; }
...
/* ─── ACTION BUTTONS (maintenance table) ─── */
.maint-actions { display:flex; gap:6px; align-items:center; flex-wrap:nowrap; }

.btn-respond {
    display:inline-flex; align-items:center; gap:5px;
    height:31px; padding:0 13px; font-size:.76rem; font-weight:800;
    border-radius:7px; cursor:pointer; transition:all .18s; white-space:nowrap;
    font-family:'Montserrat', sans-serif; border:1.5px solid;
}
.btn-respond-blue   { background:var(--blue-lt);  color:var(--blue);  border-color:#bfdbfe; }
.btn-respond-blue:hover  { background:var(--blue);  color:#fff; border-color:var(--blue);  box-shadow:0 2px 8px rgba(37,99,235,.2); }
.btn-respond-amber  { background:#fffbeb; color:#b45309; border-color:#fde68a; }
.btn-respond-amber:hover { background:var(--amber); color:#fff; border-color:var(--amber); }
.btn-respond-green  { background:var(--green-lt); color:var(--green); border-color:#99f6e4; }
.btn-respond-green:hover { background:var(--green); color:#fff; border-color:var(--green); }

.btn-remove {
    display:inline-flex; align-items:center; gap:4px;
    height:31px; padding:0 11px; font-size:.74rem; font-weight:800;
    border-radius:7px; cursor:pointer; transition:all .18s; white-space:nowrap;
    font-family:'Montserrat', sans-serif;
    background:var(--red-lt); color:var(--red); border:1.5px solid #fca5a5;
}
.btn-remove:hover { background:var(--red); color:#fff; border-color:var(--red); transform:translateY(-1px); box-shadow:0 2px 8px rgba(220,38,38,.2); }

.pending-chip { display:inline-flex; align-items:center; gap:6px; background:#fff; border:1.5px solid #f59e0b; color:#92400e; border-radius:20px; padding:4px 11px; font-size:.73rem; font-weight:800; cursor:pointer; transition:all .18s; font-family:'Montserrat', sans-serif; }
...
.req-btn { display:inline-flex; align-items:center; justify-content:center; gap:4px; height:30px; padding:0 12px; font-size:.75rem; font-weight:800; border-radius:6px; border:none; cursor:pointer; transition:all .18s; font-family:'Montserrat', sans-serif; white-space:nowrap; letter-spacing:.01em; }
...
/* ─── MODALS ─── */
.modal-overlay { position:fixed; inset:0; background:rgba(15,20,40,.45); backdrop-filter:blur(3px); display:flex; align-items:center; justify-content:center; z-index:1055; padding:16px; }
...
.modal-input, .modal-select, .modal-textarea { width:100%; border:1.5px solid var(--border); border-radius:var(--radius-sm); padding:10px 12px; font-size:.88rem; color:var(--ink); background:var(--surface); font-family:'Montserrat', sans-serif; transition:border-color .2s, box-shadow .2s; outline:none; }
.modal-input:focus, .modal-select:focus, .modal-textarea:focus { border-color:var(--maroon); box-shadow:0 0 0 3px rgba(141,11,65,.1); }

.complaint-modal-grid { display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:16px; }
.complaint-photo-preview { width:100%; max-height:180px; object-fit:cover; border-radius:10px; border:2px solid var(--border); display:block; }
.complaint-photo-wrap { grid-column:1/-1; background:var(--surface2); border:1.5px dashed var(--border2); border-radius:10px; padding:12px; text-align:center; }
.complaint-photo-wrap a { display:block; }
.field-group { display:flex; flex-direction:column; gap:6px; }
.status-option-grid { display:grid; grid-template-columns:1fr 1fr; gap:8px; }
.status-radio { display:none; }
.status-label { display:flex; align-items:center; gap:8px; padding:10px 12px; border:1.5px solid var(--border); border-radius:8px; cursor:pointer; font-size:.78rem; font-weight:700; color:var(--ink2); transition:all .18s; background:var(--surface); }
.status-label:hover { border-color:var(--maroon); background:var(--maroon-lt); }
.status-radio:checked + .status-label { border-color:var(--maroon); background:var(--maroon-lt); color:var(--maroon); }
.status-label .status-dot { width:10px; height:10px; border-radius:50%; flex-shrink:0; }
...
.complaint-meta-chip { display:inline-flex; align-items:center; gap:5px; background:var(--surface2); border:1px solid var(--border); border-radius:20px; padding:4px 10px; font-size:.72rem; font-weight:700; color:var(--ink3); }

/* Remarks hint */
.remarks-hint { font-size:.7rem; color:var(--ink3); margin-top:4px; display:flex; align-items:center; gap:4px; font-weight:600; }

/* Previous response preview */
.prev-response-box { background:var(--surface2); border:1px solid var(--border); border-left:3px solid var(--blue); border-radius:8px; padding:10px 12px; font-size:.8rem; color:var(--ink2); line-height:1.5; font-weight:600; margin-bottom:10px; }
.prev-response-label { font-size:.68rem; font-weight:800; text-transform:uppercase; letter-spacing:.06em; color:var(--blue); margin-bottom:5px; display:flex; align-items:center; gap:5px; }
...
.approve-pay-btn { display:inline-flex; align-items:center; gap:3px; background:#dcfce7; color:#15803d; border:1.5px solid #86efac; border-radius:6px; padding:4px 9px; font-size:.7rem; font-weight:800; cursor:pointer; transition:all .18s; font-family:'Montserrat', sans-serif; white-space:nowrap; }
.approve-pay-btn:hover { background:var(--green); color:#fff; border-color:var(--green); }
.reject-pay-btn  { display:inline-flex; align-items:center; gap:3px; background:#fee2e2; color:#991b1b; border:1.5px solid #fca5a5; border-radius:6px; padding:4px 9px; font-size:.7rem; font-weight:800; cursor:pointer; transition:all .18s; font-family:'Montserrat', sans-serif; white-space:nowrap; }
...
.toast-bar { position:fixed; top:80px; right:24px; z-index:9999; display:flex; align-items:center; gap:10px; background:#fff; border-radius:12px; padding:13px 18px; box-shadow:0 8px 28px rgba(0,0,0,.15); border-left:4px solid; font-size:.84rem; font-weight:700; color:var(--ink); animation:toastIn .3s ease, toastOut .4s 3.2s ease forwards; max-width:360px; }
.toast-bar.toast-success { border-left-color:var(--green); }
.toast-bar.toast-error   { border-left-color:var(--red);   }
...
<div class="toast-bar toast-success" id="toastBar">
    <i class="bi bi-check-circle-fill" style="color:var(--green);font-size:1.1rem"></i>
    <?= $_GET['success'] === 'maintenance_updated' ? 'Maintenance request updated &amp; tenant notified.' : 'Action completed successfully.' ?>
</div>
<?php elseif (!empty($_GET['error'])): ?>
<div class="toast-bar toast-error" id="toastBar">
    <i class="bi bi-exclamation-circle-fill" style="color:var(--red);font-size:1.1rem"></i>
    <?= htmlspecialchars($_GET['error']) ?>
    <?php if (!empty($_GET['msg'])): ?> — <?= htmlspecialchars($_GET['msg']) ?><?php endif; ?>
</div>
<?php endif; ?>
...
<div class="rm-wrapper">
    ...
    <?php
    $pending_verifications = 0; $overdue_count = 0;
    ...
    <div class="section-card">
        ...
        <div class="tbl-wrap">
            <table class="rm-table">
                <thead><tr><th>Tenant</th><th>Rent / mo</th><th>Lease</th><th>Last Payment</th><th>Pending</th><th>Requests</th></tr></thead>
                <tbody>
        ...
        <div class="empty-state">
            ...
            <div class="empty-state-desc">Once you approve applications, your active tenants will appear here.</div>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="section-card">
        ...
        <div class="tbl-wrap">
            <table class="rm-table">
                <thead>
                    <tr>
                        <th>Tenant</th><th>Title</th><th>Category</th><th>Priority</th>
                        <th>Status</th><th>Requested</th><th>Scheduled</th><th>Completed</th>
                ...
                <?php foreach ($complaints as $complaint):
                    ...
                    /* safe encode for data-* — use actual DB column names */
                    $cRemarks   = htmlspecialchars($complaint['landlord_remarks']  ?? '', ENT_QUOTES);
                    $cResponse  = htmlspecialchars($complaint['landlord_response'] ?? '', ENT_QUOTES);
                    $cSchedVal  = $complaint['scheduled_date'] ? date("Y-m-d", strtotime($complaint['scheduled_date'])) : '';
                ?>
                <tr>
                    <td>
                    ...
                    <td>
                        ...
                        <?php if ($cs === 'completed'): ?>
                            <button class="btn-remove remove-complaint-btn" data-id="<?= $complaint['complaint_id'] ?>">
                                <i class="bi bi-trash3-fill"></i> Remove
                            </button>
                        <?php else:
                            /* choose button colour by current status */
                            if ($cs === 'in progress') {
                                $btnClass = 'btn-respond-amber'; $btnIcon = 'bi-pencil-fill'; $btnLabel = 'Update';
                            } elseif ($cs === 'pending') {
                                $btnClass = 'btn-respond-blue'; $btnIcon = 'bi-reply-fill'; $btnLabel = 'Respond';
                            } else {
                                $btnClass = 'btn-respond-blue'; $btnIcon = 'bi-reply-fill'; $btnLabel = 'Re-respond';
                            }
                        ?>
                            <button class="btn-respond <?= $btnClass ?> respond-btn"
                                ...
                                data-remarks="<?= $cRemarks ?>"
                                data-response="<?= $cResponse ?>"
                                data-scheduled="<?= $cSchedVal ?>">
                                <i class="bi <?= $btnIcon ?>"></i> <?= $btnLabel ?>
                            </button>
                            <?php if ($cs === 'rejected'): ?>
                            <button class="btn-remove remove-complaint-btn"
                                data-id="<?= $complaint['complaint_id'] ?>"
                                style="width:31px;padding:0;justify-content:center">
        ...
        <div class="empty-state">
            ...
            <div class="empty-state-desc">Once tenants submit maintenance requests, they will appear here.</div>
        </div>
        <?php endif; ?>
    </div>
        ...
        <form id="complaintForm" method="post" action="maintenance-respond.php">
            ...
            <div class="modal-body" style="padding:20px 22px">
                ...
                <div id="compDescSection" style="display:none;margin-bottom:16px">
                    ...
                    <div id="compDescText"
                        style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:10px 12px;font-size:.85rem;color:var(--ink2);line-height:1.5;font-weight:600"></div>
                </div>

                <!-- Previous landlord response preview -->
                <div id="prevResponseWrap" style="display:none;margin-bottom:16px">
                    <div class="prev-response-label">
                        <i class="bi bi-chat-left-dots-fill"></i> Your Previous Response
                    </div>
                    <div class="prev-response-box" id="prevResponseText"></div>
                </div>
                ...
                <div style="margin-bottom:16px">
                    ...
                    <div class="status-option-grid">
                        <div>
                            ...
                            <label for="s_pending" class="status-label"><span class="status-dot sdot-pending"></span> Pending</label>
                        </div>
                        <div>
                            <input type="radio" name="status" value="in progress" id="s_progress" class="status-radio">
                            <label for="s_progress" class="status-label"><span class="status-dot sdot-progress"></span> In Progress</label>
                        </div>
                        <div>
                            <input type="radio" name="status" value="completed" id="s_completed" class="status-radio">
                            <label for="s_completed" class="status-label"><span class="status-dot sdot-completed"></span> Completed</label>
                        </div>
                        <div>
                            <input type="radio" name="status" value="rejected" id="s_rejected" class="status-radio">
                            <label for="s_rejected" class="status-label"><span class="status-dot sdot-rejected"></span> Rejected</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="complaint-modal-grid">
                    ...
                    <div class="field-group">
                        ...
                        <input type="text" class="modal-input" name="landlord_remarks" id="compRemarks"
                            placeholder="e.g., Parts ordered">
                        <div class="remarks-hint">
                            <i class="bi bi-lock-fill" style="font-size:.65rem;color:var(--amber)"></i>
                            Saved — clear field to remove
                        </div>
                    </div>
                </div>
                ...
                <div class="field-group">
                    ...
                    <textarea class="modal-textarea" name="response" id="compResponse" rows="4"
                        placeholder="Write your response… The tenant will see this message."
...
<div class="modal-overlay" id="requestModal" style="display:none" onclick="if(event.target===this)closeModal('requestModal')">
    <div class="modal-box" onclick="event.stopPropagation()">
        ...
        <div class="modal-foot">
            <button type="button" class="mbtn mbtn-cancel" onclick="closeModal('requestModal')"><i class="bi bi-x"></i> Cancel</button>
            <button class="mbtn mbtn-approve" id="approveBtn"><i class="bi bi-check-circle-fill"></i> Approve</button>
            <button class="mbtn mbtn-reject"  id="rejectBtn"><i class="bi bi-x-circle-fill"></i> Reject</button>
        </div>
    </div>
</div>
...
<div class="modal-overlay" id="paymentHistoryModal" style="display:none" onclick="if(event.target===this)closeModal('paymentHistoryModal')">
    <div class="modal-box modal-box-xl" onclick="event.stopPropagation()">
        ...
        <div class="hist-stat-grid">
            <div class="hist-stat hs-green"><div class="hist-stat-label">Total Paid</div><div class="hist-stat-val" id="histTotalPaid">₱0.00</div><div class="hist-stat-sub">All paid records</div></div>
            <div class="hist-stat hs-blue"><div class="hist-stat-label">Transactions</div><div class="hist-stat-val" id="histTotalCount">0</div><div class="hist-stat-sub" id="histPaidPending">0 paid · 0 pending</div></div>
            <div class="hist-stat hs-amber"><div class="hist-stat-label">Last Payment</div><div class="hist-stat-val" id="histLastPayment" style="font-size:.95rem">—</div><div class="hist-stat-sub">Most recent</div></div>
            <div class="hist-stat hs-red"><div class="hist-stat-label">Overdue</div><div class="hist-stat-val" id="histOverdueCount">0</div><div class="hist-stat-sub">Record(s)</div></div>
            <div class="hist-stat hs-purple"><div class="hist-stat-label">Awaiting Review</div><div class="hist-stat-val" id="histVerifyCount">0</div><div class="hist-stat-sub">Submitted by tenant</div></div>
        </div>
        <div class="hist-filters">
            <input type="text" id="histSearch" class="hist-input hist-input-search" placeholder="🔍 Search reference, method, remarks…" oninput="filterHist()">
            <select id="histStatusFilter" class="hist-input" onchange="filterHist()">
                ...
                <option value="pending_verification">Awaiting Review</option><option value="rejected">Rejected</option>
            </select>
            <select id="histTypeFilter" class="hist-input" onchange="filterHist()">
                <option value="">All Types</option>
                <option value="rent">Rent</option><option value="deposit">Deposit</option><option value="penalty">Penalty</option>
            </select>
        </div>
        <div class="hist-tbl-wrap">
            <table class="hist-tbl">
                <thead><tr><th>#</th><th>Type</th><th>Due Date</th><th>Date Paid</th><th>Amount</th><th>Method</th><th>Ref No.</th><th>Proof</th><th>Status</th><th>Remarks</th><th>Action</th></tr></thead>
                <tbody id="histTableBody"><tr><td colspan="11" style="text-align:center;padding:48px;color:var(--ink3)">No records found.</td></tr></tbody>
            </table>
        </div>
        <div class="hist-foot">
        ...
        <div class="modal-foot">
            <button type="button" class="mbtn mbtn-cancel" onclick="closeModal('paymentHistoryModal')"><i class="bi bi-x"></i> Close</button>
        </div>
    </div>
</div>
...
const toastEl = document.getElementById('toastBar');
if (toastEl) setTimeout(() => toastEl.remove(), 3800);
...
document.querySelectorAll('.respond-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        if (desc) { document.getElementById('compDescText').textContent = desc; section.style.display='block'; }
        ...
        const radio = document.getElementById(statusMap[(this.dataset.status||'pending').toLowerCase()] || 's_pending');
        if (radio) radio.checked = true;

        /* ── persistent fields ── */
        document.getElementById('compScheduledDate').value = this.dataset.scheduled || '';
        document.getElementById('compRemarks').value       = this.dataset.remarks   || '';

        /* show previous response as read-only preview */
        const prevResp = this.dataset.response || '';
        const prevWrap = document.getElementById('prevResponseWrap');
        if (prevResp) {
            document.getElementById('prevResponseText').textContent = prevResp;
            prevWrap.style.display = 'block';
        } else { prevWrap.style.display = 'none'; }

        /* clear textarea so landlord types a fresh message */
        const respEl = document.getElementById('compResponse');
        respEl.value = '';
        respEl.placeholder = prevResp ? 'Type a new update for the tenant…' : 'Write your response… The tenant will see this message.';
...
document.querySelectorAll('.remove-complaint-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        Swal.fire({ title:'Remove this request?', text:'This cannot be undone.', icon:'warning',
            showCancelButton:true, confirmButtonColor:'#dc2626', cancelButtonColor:'#718096', confirmButtonText:'Yes, Remove'
        }).then(r => {
            if (!r.isConfirmed) return;
            fetch('maintenance-delete.php', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'complaint_id='+id })
...
document.querySelectorAll('.renewal-btn, .termination-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        const type = this.dataset.type, reason = this.dataset.reason||'', date = this.dataset.date||'';
...
let histAllRows = [];
function fmtDate(d){if(!d)return'—';const dt=new Date(d);return isNaN(dt.getTime())?'—':dt.toLocaleDateString('en-PH',{month:'short',day:'numeric',year:'numeric'});}
function fmtAmt(a){return'₱'+parseFloat(a||0).toLocaleString('en-PH',{minimumFractionDigits:2});}
function daysLate(ds){if(!ds)return null;const due=new Date(ds),now=new Date();now.setHours(0,0,0,0);due.setHours(0,0,0,0);return now<=due?null:Math.floor((now-due)/86400000);}
function statusBadge(s){const m={paid:['hb-paid','bi-check-circle-fill','Paid'],partial:['hb-partial','bi-dash-circle-fill','Partial'],pending:['hb-pending','bi-clock-fill','Pending'],overdue:['hb-overdue','bi-exclamation-circle-fill','Overdue'],pending_verification:['hb-verify','bi-hourglass-split','Awaiting Review'],rejected:['hb-rejected','bi-x-circle-fill','Rejected']};const[cls,ico,lbl]=m[s]||['hb-pending','bi-question-circle',s||'Unknown'];return`<span class="hbadge ${cls}"><i class="bi ${ico}"></i>${lbl}</span>`;}
function typePill(t){const m={rent:['ht-rent','bi-house-fill','Rent'],deposit:['ht-deposit','bi-safe2-fill','Deposit'],penalty:['ht-penalty','bi-exclamation-triangle-fill','Penalty']};const[cls,ico,lbl]=m[t]||['ht-other','bi-tag-fill',t||'Other'];return`<span class="htype ${cls}"><i class="bi ${ico}"></i>${lbl}</span>`;}
function methodPill(m){if(!m)return'<span style="color:var(--ink3)">—</span>';return`<span style="background:var(--surface2);color:var(--ink2);padding:3px 9px;border-radius:20px;font-size:.72rem;font-weight:700;display:inline-flex;align-items:center;gap:4px"><i class="bi bi-credit-card"></i>${m.replace(/_/g,' ')}</span>`;}

function renderHistTable(rows){
    const tbody=document.getElementById('histTableBody');
    if(!rows.length){tbody.innerHTML=`<tr><td colspan="11" style="text-align:center;padding:48px;color:var(--ink3);font-size:.85rem"><i class="bi bi-receipt" style="font-size:2.5rem;display:block;margin-bottom:12px;opacity:.3"></i>No records found.</td></tr>`;document.getElementById('histRowCount').textContent='0 transaction(s)';document.getElementById('histFooterTotal').textContent='₱0.00';return;}
    ...
    tbody.innerHTML=rows.map((p,i)=>{
        ...
        return`<tr class="${rc}"><td style="color:var(--ink3);font-size:.75rem">${i+1}</td><td>${typePill((p.payment_type||'').toLowerCase())}</td><td>${dueHtml}</td><td>${p.paid_date?fmtDate(p.paid_date):'<span style="color:var(--ink3)">Not yet paid</span>'}</td><td style="font-weight:900;color:var(--ink)">${p.amount!=null?fmtAmt(p.amount):'—'}</td><td>${methodPill(p.payment_method)}</td><td>${p.reference_no?`<span class="ref-mono">${p.reference_no}</span>`:'<span style="color:var(--ink3)">—</span>'}</td><td>${proof}</td><td>${statusBadge(s)}</td><td style="max-width:140px;font-size:.76rem;color:var(--ink3);white-space:normal;font-weight:600">${p.remarks||'<span style="color:var(--ink3)">—</span>'}</td><td>${action}</td></tr>`;
    }).join('');
...
function reviewPayment(id,action){
    Swal.fire({title:(action==='approved'?'Approve':'Reject')+' this payment?',text:action==='approved'?'This will mark the payment as Paid.':"This will reject the tenant's payment.",icon:action==='approved'?'question':'warning',showCancelButton:true,confirmButtonColor:action==='approved'?'#0d9488':'#dc2626',cancelButtonColor:'#718096',confirmButtonText:action==='approved'?'Yes, Approve':'Yes, Reject'}).then(r=>{
        if(!r.isConfirmed)return;
        fetch('review-payment.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`payment_id=${id}&action=${action}`}).then(r=>r.text()).then(raw=>{
            let data;try{data=JSON.parse(raw);}catch{Swal.fire({icon:'error',title:'Error',text:'Unexpected response.',confirmButtonColor:'#8d0b41'});return;}
            Swal.fire({icon:data.success?'success':'error',title:data.success?'Done!':'Failed',text:data.success?(action==='approved'?'Payment approved.':'Payment rejected.'):(data.message||'Something went wrong.'),confirmButtonColor:'#8d0b41'}).then(()=>{if(data.success)location.reload();});
        }).catch(e=>Swal.fire({icon:'error',title:'Network Error',text:e.message,confirmButtonColor:'#8d0b41'}));
    });
}
...
document.querySelectorAll('.view-history-btn').forEach(btn=>{
    btn.addEventListener('click',function(){
        const tid=this.dataset.tenantId,name=this.dataset.tenantName,prop=this.dataset.property,rent=parseFloat(this.dataset.amount||0),records=paymentHistoryData[tid]||[],pre=this.dataset.filterStatus||'';
        histAllRows=records;
